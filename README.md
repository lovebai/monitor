...
- 主页（5 秒局部刷新，不整页刷新）：
  - 实时更新节点状态、CPU/内存/磁盘、负载（按核数归一化百分比 + 原始值）、网络速率（已启用网卡汇总）、探测延迟、开机时长（如 3天15时）、Agent 系统时间与告警；
...
- 主页按分组展示节点卡片，顶部汇总在线数、上下行速率与活动告警数；卡片底部浅色显示开机时长（如 3天15时）与 Agent 系统时间；5 秒局部刷新，无需整页刷新。
