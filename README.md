...
- 主页（5 秒局部刷新，不整页刷新；新增节点/分组与「尚未收到 Agent 上报」空态也会自动更新）：
  ...
  - 页面底部展示 Server 主机状态：系统、负载、CPU、内存、磁盘使用率与数据库文件大小（约 10 秒采样、随 5 秒刷新更新）。
...
- 主页底部「Server 主机状态」显示服务端所在设备的系统、负载、CPU、内存、磁盘使用情况及数据库文件大小（Windows 无原生负载均值，按 CPU 使用率估算，与 Agent 口径一致）。
