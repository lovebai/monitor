...
- 采集：主机、OS、CPU 型号/核心数、内存、磁盘、负载、网卡（MAC/MTU/状态/IP/流量速率）、TCP 探测延迟、进程与服务健康检查。
...
- 节点详情页：摘要卡片、内存、磁盘（全部分区）、已启用网卡（仅名称/MAC/IPv4）、服务与进程表格、告警，5 秒局部刷新。
...
- Linux：完整采集 CPU、内存、磁盘、负载与网卡流量（`/proc`、`df` 等）。
- Windows：内置 CIM 采集 CPU、内存、磁盘；网卡字节速率通过 `Get-NetAdapterStatistics` 采集；负载以 CPU 利用率估算（Windows 无原生负载均值）。
...
- 详情页：摘要卡片、内存、磁盘（全部分区）、网卡（仅已启用网卡的名称/MAC/IPv4）、服务与进程表格、告警，全部 5 秒局部刷新。
